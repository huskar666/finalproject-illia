...
const searchInput = document.querySelector(".header__div--navigation--inputs--input--text1");


const countryOptions = document.querySelectorAll(".header__div--navigation--inputs--input2--option");
const selectedCountryText = document.querySelector(".header__div--navigation--inputs--input2--selected-text");
const optionsDropdown = document.querySelector(".header__div--navigation--inputs--input2--options");

document.querySelector(".header__div--navigation--inputs--input2--selected").addEventListener("click", () => {
  optionsDropdown.style.display = optionsDropdown.style.display === "flex" ? "none" : "flex";
});

countryOptions.forEach(option => {
  option.addEventListener("click", () => {
    currentCountryCode = option.getAttribute("data-country-code");
    selectedCountryText.textContent = option.textContent;
...
document.addEventListener("click", e => {
  if (!e.target.closest(".header__div--navigation--inputs--input2")) {
    optionsDropdown.style.display = "none";
  }
});

async function getEvents(page = 1) {
  const apiKey = "49FrD8b5pF7reRwv5Ebt667wyQ9AQQPZ";
  const API_URL = `https://app.ticketmaster.com/discovery/v2/events.json?limit=${cardsPerPage}&page=${page - 1}&countryCode=${currentCountryCode}&segmentName=Film&apikey=${apiKey}`;
  try {
...
cardsContainer.addEventListener("click", (e) => {
  ...
  let card = e.target.closest(".main__container--div--card");
  if (card) {
